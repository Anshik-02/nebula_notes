...
jest.mock('../quickActions',(() => () => <div>Mocked QuickActions</div>));

describe("this component gets 3 Fav of user and displays then on dashboard", () => {
  beforeEach(() => {
  ...
  test("renders 3 fav for the user", async () => {
const ui = await FavNotes();
  render(ui);

    expect(await screen.findByText("Favorite Notes")).toBeInTheDocument();
    ...
    expect(await screen.findByText("Mocked QuickActions")).toBeInTheDocument();
